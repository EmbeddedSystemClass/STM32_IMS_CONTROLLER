...
void FrequencyMeasureInit(void)
{
	 ...
	 TIM_ITConfig(FREQ_TIM,TIM_IT_CC1, ENABLE);
...
void Freq_TIM_IRQHandler()
{
	 if (FREQ_TIM->SR & TIM_IT_CC1 )
	 {
		 ...
		 FrequencyDataCH1[FrequencyDataCH1counter].capture =(uint32_t)FREQ_TIM->CCR1|(((uint32_t)TIM2->CCR3)<<16);
		 FrequencyDataCH1[FrequencyDataCH1counter].impulse_count=TIM1->CNT;
		 TIM1->CNT=0x0;
